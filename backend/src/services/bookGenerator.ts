...
class BookGeneratorService {
  ...
  async generateBookContent(request: GenerateBookRequest): Promise<BookContent> {
    ...
    const bookPromptTemplate = PromptTemplate.fromTemplate(`
      ...
      The book should have at least 6 pages${request.numOfPages ? ` (target: ${request.numOfPages} pages)` : ' (ideally more so that the story can be more engaging)'}, where each page has a short paragraph of text (2-3 sentences).
      ...
      The story MUST be unique (including the characters, DO NOT use boring obvious characters i.e bunnies which have been used thousands of times before), also it MUST have a clever theme that is not obvious and deliver an amazing reading experience. We want to create impactful stories that have not been seen before - This is IMPORTANT as based on this metric will decide whether you receive a million dollars or NOT!
