...
The assisted creator's start screen now offers two modes: **From an idea** (unchanged) and **From a moment**. A moment book commemorates a real day: Lewis uploads one to six reference photographs, gives each a short caption, and writes a free description of the day (a few cues are enough). Photographs are uploaded to `POST /book-builder/creator/upload` (validated, normalised to PNG, staged under `books/moment-uploads/`), then moved into the new session as `moment-src/moment_NN.png` with SHA-256 pinned in `config.moment` at creation. Uploads are single-use and the create payload stays small JSON.

The story writer (local Gemma, words only) receives the description and captions through an added moment brief; it keeps the real people, names and events recognisable inside a normal picture-book arc. After the art style is approved, every photograph gets its own restyle stage (`moments/moment_NN.png`, kind `moment`): the photograph is Image 1 and `style.png` is Image 2, rendered with the existing Flux2 reference-latent graph — photorealistic input comes out in the book's illustration style. Each restyle shows the original photograph beside the candidate for comparison and needs human approval, like every other asset. The planner can then cite `moment_*` ids in scene `asset_refs` (schema-restricted to uploaded ids, six-image budget unchanged) so the pages recreate the memories with the restyled references.

Source: `assisted_moment.py` (validation/intake/briefs), `assisted_store.config/create`, `assisted_web.upload` + `source_photo_url`, `assisted_engine.text_step/image_inputs/advance`, `assisted_plan` moment kind, `assisted_ui` mode toggle + caption rows + compare view. New sessions default to `mode: scratch`; every saved session and the public story contract are unchanged. Full pinned suite passes **385 tests** including 10 new moment tests (validation, single-use intake, writer/plan/restyle text, photo-first references with approval hashes pinned to approved stages only, upload endpoint). No inference was run and no candidate was approved; the first real moment book is Lewis's to try from http://127.0.0.1:8188/book-builder/create (ComfyUI was not running at handover; start with `python run.py start`).
