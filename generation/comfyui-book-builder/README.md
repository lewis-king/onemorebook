...
3. Generate an empty style reference and a plain-background canonical portrait for every character. Portraits use permanent appearance only; behavioural notes and later story marks cannot define the baseline. Each reference must pass visual review.
   In from-a-moment books, no photograph is restyled upfront: each page that illustrates a memory receives the original photograph as a direct image input at render time (the art style arrives as words and from the approved references), and the original appears beside each page candidate until you approve it. Only reusable things — characters, props, places and their states — are generated and reviewed as references upfront.
