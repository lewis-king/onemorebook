...
- Plan: `moment` asset kind restricted by schema to the uploaded ids; planner guidance lists available moments with captions and requires every uploaded photograph to illustrate the cover or a page — validation rejects a plan that leaves one unused, and moment-cited scene briefs bind the render to the memory ("recreate Image N faithfully — same people, poses, key objects and setting") so the illustration keeps what actually happened; six-image budget per scene unchanged.
