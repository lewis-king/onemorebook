...
- Plan: `moment` asset kind restricted by schema to the uploaded ids; planner guidance lists available moments with captions and requires every uploaded photograph to illustrate the cover or a page — validation rejects a plan that leaves one unused, reuses one, puts two on a page, or orders pages against the upload (day) order, and moment-cited scene briefs bind the render to the memory ("recreate Image N faithfully — same people, poses, key objects and setting") so the illustration keeps what actually happened; six-image budget per scene unchanged. The form numbers each photo row with Move up/down arrows because upload order is the day's chronology (cake last).
