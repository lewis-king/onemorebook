...
**Judge model selection, benchmarked** (harness: `.local/judge-bench/bench.py`, nine real cases with known-correct verdicts from the first live book): gemma4:31b 8/9 (zero false rejects, but misranks the four-image style comparison), qwen3.5-27b 8/9 (ranks style takes correctly, but false-rejects good pages), qwen3-vl:32b 6/9 (over-strict, one truncated review). So `review_model` stays gemma4:31b for story/plan/reference/page judging and `style_review_model` defaults to Qwen3.5-27B-GGUF for the style-take ranking only; the style prompt also guards explicitly against photo-of-a-print takes and corner-dwelling animals. Verified wired: the real style pool is correctly rejected with per-take reasons (photo+cat, cat, cat).

Proven live end to end: book-20260919123550-4f152ddd03 ("The Blanket Palace", 4 pages, 17 stages) ran creation → complete in ~35 min with zero human input: 28 yolo decisions (10 rejects that visibly fixed the next attempt — one-off props dropped from the plan, baked-in props removed from character portraits, off-model pyjamas/pigtails and wrong cat fur caught on page 2). Kimi adjudication of every stage agreed with the judge except the style-take occupancy slip (fixed in the prompt; server restarted). Post-completion surgery verified: human reopen + regenerate of page 2 → judge approved attempt 4 → new immutable `exports/b7de1caa01e1971b/`. Pause action verified via API. Full pinned suite **419 tests pass** (15 in `tests/test_assisted_yolo.py`).
...
Source: `assisted_engine.image_inputs`, `assisted_prompt_context.revision_request`, `assisted_moment.writer_brief`, `tests/test_moment_books.py`. Full pinned suite **404 tests pass**; ComfyUI restarted idle.

---

# Moments render directly into pages — 2026-09-13
